  ...
  return (
    <section className="py-16 sm:py-24 bg-secondary/50">
      <div className="container">
        ...
        <StaggerChildren className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-5">
          {categories.map((cat) => (
            <motion.div key={cat.slug} variants={staggerItemVariants}>
              <Link
                to={`/tours?category=${cat.slug}`}
                className="group relative rounded-2xl overflow-hidden aspect-[3/2] shadow-card hover:shadow-elevated transition-all duration-500 hover:-translate-y-1.5 active:scale-[0.97] block"
              >
                <img
                  src={cat.image}
                  alt={cat.name}
